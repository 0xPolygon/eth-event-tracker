...
func testFilter(t *testing.T, provider Provider, filterConfig *FilterConfig) []*ethgo.Log {
	tt, err := NewTracker(provider, WithFilter(filterConfig))
	require.NoError(t, err)

	tt.blockTracker, err = blocktracker.NewBlockTracker(provider)
	require.NoError(t, err)

	if err := tt.syncImpl(context.Background()); err != nil {
		t.Fatal(err)
	}
...
func TestTracker_Sync_Restart(t *testing.T) {
	...
	advance := func(first, last int, void ...bool) {
		...
		tt.blockTracker, err = blocktracker.NewBlockTracker(m)
		require.NoError(t, err)
...
func testSyncerReconcile(t *testing.T, iniLen, forkNum, endLen int) {
	...
	tt0.blockTracker, err = blocktracker.NewBlockTracker(m)
	require.NoError(t, err)

	err = tt0.syncImpl(context.Background())
	assert.NoError(t, err)
	...
	tt1.blockTracker, err = blocktracker.NewBlockTracker(m1)
	require.NoError(t, err)
...
func testTrackerSyncerRandom(t *testing.T, n int, backlog uint64) {
	...
	for i := 0; i < n; i++ {
		...
		tt.blockTracker, err = blocktracker.NewBlockTracker(m, blocktracker.WithBlockMaxBacklog(backlog))
		require.NoError(t, err)
...
func TestTracker_Reconcile(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.Name, func(t *testing.T) {
			...
			tt, err := NewTracker(m, WithStore(store))
			assert.NoError(t, err)
...
func TestTracker_TooMuchDataRequested(t *testing.T) {
	count := 0
	...
	blockTracker, err := blocktracker.NewBlockTracker(m)
	require.NoError(t, err)
