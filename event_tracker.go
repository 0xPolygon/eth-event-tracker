...
// SyncCh is a channel that gets closed when the bulk sync is done
func (t *Tracker) SyncCh() <-chan struct{} {
	return t.syncCh
}

func (t *Tracker) EventCh() <-chan *Event {
	return t.eventCh
}
...
START:
	...
	if err := t.storeEvent(evnt); err != nil {
		return err
	}
...
func (t *Tracker) Sync(ctx context.Context) error {
	...
	// start the block tracker and subscription
	blockTracker, err := blocktracker.NewBlockTracker(t.provider)
	if err != nil {
		return err
	}
	t.blockTracker = blockTracker
	go blockTracker.Start(ctx)
	...
	// watch sync
	for {
		event, err := t.blockTracker.Next(ctx)
		if err != nil {
			return err
		}
		if _, err := t.handleBlockEvent(event); err != nil {
			return err
		}
	}
}
...
func (t *Tracker) syncImpl(ctx context.Context) error {
	...
	headBlock := t.blockTracker.Flush().Header()
	if headBlock == nil {
		return nil
	}
	...
	var origin uint64
	if last != nil {
		if last.Number > headBlock.Number {
			return fmt.Errorf("store '%d' is more advanced than the head chain block '%d'", last.Number, headBlock.Number)
		}
		...
		if pivot.Hash != last.Hash {
			...
			if err := t.storeEvent(&Event{Indx: int64(indx)}); err != nil {
				return err
			}
		}
	}

	if headBlock.Number-origin+1 > t.config.MaxBacklog {
		...
		for {
			if origin > headBlock.Number {
				return fmt.Errorf("from (%d) higher than to (%d)", origin, headBlock.Number)
			}
			if headBlock.Number-origin+1 <= t.config.MaxBacklog {
			...
			target := headBlock.Number - t.config.MaxBacklog
	...
	added := []*ethgo.Block{headBlock}

	for headBlock.Number != origin {
		headBlock, err = t.provider.GetBlockByHash(headBlock.ParentHash, false)
		if err != nil {
			return err
		}
		added = append(added, headBlock)
	}
...
func (t *Tracker) storeEvent(event *Event) error {
	if err := t.entry.StoreEvent(event); err != nil {
		return err
	}
...
func (t *Tracker) handleBlockEvent(blockEvnt *blocktracker.BlockEvent) (*Event, error) {
	...
	for _, block := range blockEvnt.Added {
		...
		// Some nodes might not have yet this hash, try again a few times
		for i := 0; i < 5; i++ {
			logs, err = t.provider.GetLogs(query)
			if err == nil {
				break
